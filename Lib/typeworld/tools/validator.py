...
profiles = (
	('setup', 'Setup', 'Check API endpoint response (`EndpointResponse` class) and download subscription (`InstallableFontsResponse` class).\nIf necessary, create a Type.World user account and try again.'),
	('freefont', 'Free Fonts', 'Download and install a free font. This subscription needs to hold at least one free font.\nThe first free font will be downloaded'),
	('nonexpiringprotectedfont', 'Non-expiring Protected Fonts', 'To check the proper response of your API Endpoint in handling protected fonts (the norm for commercial setups),\nthis test requires the subscription to hold at least one protected font with at least one `LicenseUsage` object\nwith a `seatsAllowed` attribute of `1` and a `seatsInstalled` attribute of `0`.\nThe routine will install the font, and then attempt to install the same font a second time on\nanother (assumed) computer. This is supposed to fail because only 1 seat is allowed.\nOnly after removing the font installation, the second computer can succeed in installing the font.'),
#	('expiringprotectedfont', 'Expiring Protected Fonts', 'To check the proper response of your API Endpoint in handling protected fonts (the norm for commercial setups), this test requires the subscription to hold at least one protected font with at least one LicenseUsage object with a seatsAllowed attribute of 1 and a seatsInstalled attribute of 0. The routine will install the font, and then attempt to install the same font a second time on another (assumed) computer. This is supposed to fail because only 1 seat is allowed. Only after removing the font installation, the second computer can succeed in installing the font.'),
)


def validateAPIEndpoint(subscriptionURL, runProfiles, endpointURL = 'https://api.type.world/v1', responses = {}):

	if 'freefont' in runProfiles and not 'setup' in runProfiles:
		runProfiles.append('setup')

	if 'nonexpiringprotectedfont' in runProfiles and not 'setup' in runProfiles:
		runProfiles.append('setup')

	if 'all' in runProfiles:
		for keyword in [x[0] for x in profiles]:
			if not keyword in runProfiles:
...
def main():
	...
	for keyword, title, description in profiles:
		...
		helpString += f'\n\n{headline}\n{"#" * len(headline)}\n{description}'
	parser = argparse.ArgumentParser(description='Validate a Type.World API JSON Endpoint', formatter_class=argparse.RawTextHelpFormatter)
	parser.add_argument('subscriptionURL', metavar='subscriptionURL', type=str, help='Type.World Subscription URL (see: https://type.world/developer#the-subscription-url)')
