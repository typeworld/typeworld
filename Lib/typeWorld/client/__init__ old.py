...
class APIRepository(object):
	...
	def updateWithAPIObject(self, api):
		...
		# Save preferences
		self.parent.set('versions', [x.dumpDict() for x in self.repositoryVersions])
...
class APIEndPoint(object):
	...
	def get(self, key):
		preferences = self.parent.preferences.get(self.canonicalURL)
	...
	def set(self, key, value):
		...
		self.parent.preferences.set(key, value)
...
#		self.loadPreferences()
	...
	def addRepository(self, url):
		...
		# Add endpoint if new
		endpointlist = self.preferences.get('endpoints')
		if endpointlist:
			endpointlist.append(api.canonicalURL)
		else:
			endpointlist = [api.canonicalURL]
		self.preferences.set('endpoints', endpointlist)
...
#		return True, None, self.endpoints[api.canonicalURL]
	...
	def printPreferences(self):
		...
		for key in ['endpoints']:
			print '#### Key:', key
			url = self.preferences.get(key)

			if url:
				print self.preferences.get(url)
...
if __name__ == '__main__':
	...
	client.addRepository('http://192.168.56.102/type.world/api/wsqmRxRmY3C8vtrutfIr/?command=installableFonts&userID=K0TUTPgE4Kp6vjDOfT47')

	client.printPreferences()
