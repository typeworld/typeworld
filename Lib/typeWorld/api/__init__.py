...
#		'public':			[BooleanDataType,		False, 	False, 	u'If false, signals restricted access to a commercial font only available to certain users. Download and installation may be restricted by the API point depending on the API point URL that needs to include the private key to identify the user.'],
		'type':				[FontTypeDataType,		True, 	None, 	u'Technical type of font. This influences how the app handles the font. For instance, it will only install desktop fonts on the system, and make other font types available though folders. Possible: %s' % (FONTTYPES.keys())],
		'fileExtension':	[FontExtensionDataType,	False, 	None, 	u'File extension. Required in case of `desktop` font (see ::Font.type::. Possible: %s' % FILEEXTENSIONS],
		'trackSeatAllowance':[BooleanDataType,		False, 	None, 	u'Indication that the server tracks font installations and a userID needs to be used for authentication. The server *may* limit the downloads of fonts. This may also be used for fonts that are free to download, but their installations want to be tracked/limited anyway.'],
